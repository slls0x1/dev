...
grep -qxF "server 172.16.1.1 iburst" /etc/chrony.conf 2>/dev/null || { echo "server 172.16.1.1 iburst" >> /etc/chrony.conf 2>/dev/null || { echo "[ ERROR ] Failed to append NTP server configuration"; exit 1; }; }
...
chronyc sources -v >/dev/null 2>&1 || { echo "[ ERROR ] chronyc sources command failed"; exit 1; }
