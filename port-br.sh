...
iptables -t nat -A PREROUTING -i enp7s1 -p tcp --dport 2026 -j DNAT --to-destination 192.168.3.10:2026 2>/dev/null || { echo "[ ERROR ] Failed to apply DNAT rule for port 2026"; exit 1; }
iptables -t nat -A PREROUTING -i enp7s1 -p tcp --dport 8080 -j DNAT --to-destination 192.168.3.10:8080 2>/dev/null || { echo "[ ERROR ] Failed to apply DNAT rule for port 8080"; exit 1; }
