...
grep -qxF "server 172.16.2.1 iburst" /etc/chrony.conf 2>/dev/null || { echo "server 172.16.2.1 iburst" >> /etc/chrony.conf || { echo "[ ERROR ] Failed to append NTP server configuration"; exit 1; }; }
